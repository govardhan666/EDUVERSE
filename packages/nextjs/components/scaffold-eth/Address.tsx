...
export const Address = ({ address, disableAddressLink, format, size = "base" }: TAddressProps) => {
  ...
  const formattedAddress = address ? (`0x${address}` as `0x${string}`) : undefined;
  const { data: fetchedEns } = useEnsName({ address: formattedAddress, enabled: isAddress(address ?? ""), chainId: 1 });
